...
def test_edit_template_name_success(
    client: TestClient, test_template: Dict
) -> None:
    """Test successful update of template name."""
    new_name = 'updated-name'
    response = client.patch(
        f"/templates/{test_template['id']}", json={'name': new_name}
    )
    assert response.status_code == status.HTTP_200_OK

    wait_for_field_value(
        client, f"/templates/{test_template['id']}/", 'status', 'available'
    )

    get_response = client.get(f"/templates/{test_template['id']}/")
...
def test_edit_template_invalid_name_too_short(
    ...
    response = client.patch(
        f"/templates/{test_template['id']}", json={'name': ''}
    )
...
def test_edit_template_invalid_name_too_long(
    ...
    long_name = 'a' * 41
    response = client.patch(
        f"/templates/{test_template['id']}", json={'name': long_name}
    )
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
def test_edit_template_empty_body(
    client: TestClient, test_template: Dict
) -> None:
    """Test that empty request body is rejected."""
    response = client.patch(f"/templates/{test_template['id']}", json={})
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
def test_edit_template_unauthorized(
    ...
    response = unauthorized_client.patch(
        f"/templates/{test_template['id']}", json={'name': 'new'}
    )
