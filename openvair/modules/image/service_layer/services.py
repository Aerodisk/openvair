...
class ImageServiceLayerManager(BackgroundTasks):
    ...
    Attributes:
        ...
        storage_service_client (RabbitRPCClient): RPC client for communicating
            with the storage service layer.
        event_client (EventCrud): RPC client for communicating
            with the eventstore service layer.
    ...
    def _check_image_status(
        image_status: str | None, available_statuses: list[str]
    ) -> None:
    ...
    def _check_available_space_on_storage(
        image_size: int | None, storage: StorageInfo
    ) -> None:
