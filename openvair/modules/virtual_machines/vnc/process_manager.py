...
from openvair.modules.virtual_machines.vnc.exceptions import WebsockifyProcessError
...
class WebsockifyProcessManager:
    ...
    def start_websockify(
        ...
        """
        LOG.info(f"Starting websockify for VM {vm_id}: {vnc_host}:{vnc_port} -> ws:{ws_port}")
        ...
        with self._process_lock:
            ...
            if vm_id in self._processes:
                ...
                if self._is_process_alive(existing_pid):
                    LOG.warning(f"WebSockify already running for VM {vm_id} (PID: {existing_pid})") 
                    return existing_pid
                else:
            ...
            try:
                ...
                result = execute(
                    ...
                    '--web', '/opt/aero/openvair/openvair/libs/noVNC/',
                    str(ws_port),
                    f'{vnc_host}:{vnc_port}',
    ...
    def stop_by_port(self, ws_port: int) -> bool:
        ...
        with self._process_lock:
            ...
            if vm_id:
                ...
                LOG.warning(f"No registered process for port {ws_port}, attempting direct kill")
                pid = self._find_websockify_pid(ws_port)
                if pid:
    ...
    def cleanup_orphaned_processes(self) -> int:
        ...
        try:
            ...
            if result.stdout.strip():
                ...
                registered_pids = {info['pid'] for info in self._processes.values()}
                
                # Find orphaned PIDs
                orphaned_pids = set(all_pids) - registered_pids
                
                for pid in orphaned_pids:
                    LOG.info(f"Cleaning up orphaned websockify process PID {pid}")
