...
This module manages allocation and deallocation of WebSocket ports for VNC sessions,
ensuring no conflicts occur when multiple VMs are started simultaneously.
"""
...
class VncPortPoolManager:
    ...
    def _ensure_directories(self) -> None:
        ...
        try:
            ...
            raise VncPortAllocationError(f"Directory setup failed: {e}")
    ...
    def _acquire_lock(self):
        ...
        try:
            # Open lock file
            lock_fd = os.open(VNC_LOCK_FILE, os.O_CREAT | os.O_RDWR, 0o644)
            ...
            raise VncPortAllocationError(f"Lock acquisition failed: {e}")
        finally:
    ...
    def _save_state(self, state: Dict) -> None:
        ...
        try:
            ...
            # Atomic rename
            os.rename(temp_file, VNC_STATE_FILE)
            
            # Set proper permissions
            os.chmod(VNC_STATE_FILE, 0o644)
    ...
    def allocate_port(self, vm_id: str) -> int:
        ...
        with self._acquire_lock():
            ...
            for port in state['free_ports'][:]:  # Copy to avoid modification during iteration
                if self._is_port_actually_free(port):
                    ...
                    LOG.warning(f"Port {port} marked as free but is occupied, removing from pool")
                    state['free_ports'].remove(port)
            
            # If we get here, no ports were actually free
            self._save_state(state)
            raise VncPortPoolExhaustedException("All ports in pool are occupied")
    ...
    def release_port(self, port: int, vm_id: Optional[str] = None) -> None:
        ...
        with self._acquire_lock():
            ...
            if vm_id and state['allocated_ports'][port_str]['vm_id'] != vm_id:
                LOG.warning(f"VM ID mismatch for port {port}: expected {vm_id}, "
