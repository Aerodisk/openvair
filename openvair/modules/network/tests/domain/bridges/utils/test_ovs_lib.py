...
from openvair.modules.network.domain.bridges.utils.exceptions import (
    OVSManagerException,
    InterfaceNotFoundException, BridgeNotFoundException
)
...
def test_edit_interface_address(ovs_manager, tmp_bridge):
    ...
    ovs_manager.edit_interface_address(interface_name, initial_ip_address,
                                       new_ip_address)
...
def test_turn_off_interface(ovs_manager, tmp_bridge):
    ...
    assert ovs_manager.check_interface_state(interface_name, {"DOWN", "UNKNOWN"})
